...
namespace SimpleHttpServer
{
    ...
    std::string to_string(const HttpResponse& response, bool send_content) {
        ...
        oss << static_cast<int>(response.status()) << ' ';
        oss << to_string(response.status()) << "\r\n";
    ...
    HttpRequest string_to_request(const std::string &request_str)
    {
        ...
        rpos = request_str.find("\r\n", lpos);
        if (rpos == std::string::npos) {
            throw std::invalid_argument("Could not find request start line");
        }

        start_line = request_str.substr(lpos, rpos - lpos);
        lpos = rpos + 2;
        rpos = request_str.find("\r\n\r\n", lpos);
        if (rpos != std::string::npos) {  // has header
            header_lines = request_str.substr(lpos, rpos - lpos);
            lpos = rpos + 4;
            rpos = request_str.length();
            if (lpos < rpos) {
            message_body = request_str.substr(lpos, rpos - lpos);
            }
        }
